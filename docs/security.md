...
mdnest is designed to be private by default. Whether you run it for yourself or for a team, this document explains how your notes stay secure at every layer.
...
**Multi mode:**
- Stored as **bcrypt hashes** in the PostgreSQL `users` table
- Initial admin credentials seeded from `mdnest.conf` on first startup
- Each user has their own credentials

In both modes, the backend logs a warning on startup if default credentials are still in use.
...
## Multi-User Security (AUTH_MODE=multi)

When multi-user mode is enabled, additional security measures apply:

- **Three-tier role hierarchy** (v3.5.0+) — see below.
- **Namespace-level access control:** Collaborators only see and access namespaces (or specific directories within namespaces) they have been granted access to.
- **Database credentials:** PostgreSQL password is stored in `.env` (generated by `setup.sh`) and never baked into Docker images.
- **Permission enforcement:** Every file operation checks the user's access grants before proceeding.
- **API tokens are scoped to creator's access at request time** (v3.5.0+). A token can never grant access broader than the user who minted it currently has. Revoking a user's grant immediately revokes their tokens for that namespace.

### Role hierarchy (v3.5.0+)

```mermaid
flowchart TD
    A[Request] --> B{Role?}
    B -->|superadmin| C[Allow — global]
    B -->|admin| D{Namespace<br/>in namespace_admins?}
    D -->|yes| C
    D -->|no| E{Has matching grant?}
    B -->|collaborator| E
    E -->|yes| C
    E -->|no| F[Deny — 403]
```
...
| **admin** | Per-namespace (one or more entries in `namespace_admins`) | Within their namespaces only: invite users (must specify a namespace they admin), create/edit/revoke grants, promote co-admins, trigger git-sync. Implicit read+write on those namespaces. **Cannot** reset 2FA, delete users, or change global roles — those are SuperAdmin-only |
| **collaborator** | Per-grant rows in `access_grants` | Read or write only on namespaces / paths they've been granted |
...
Existing pre-v3.5.0 admins are migrated to `superadmin` automatically by migration 007 so they keep current behaviour. New admins post-upgrade are namespace-scoped — promoted via `POST /api/admin/namespace-admins` or the admin panel's "Namespace Admins" tab.
...
- No multi-factor authentication (protected by network layer; consider Tailscale ACLs for additional control)

These are intentional trade-offs for a privately-hosted app. The network layer (localhost + Tailscale) is the primary security boundary.
