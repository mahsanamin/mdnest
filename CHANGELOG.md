...
- **Live editor: much tighter left gutter + a toggle to remove it entirely.** Crepe reserved an ~88px left gutter for its hover drag/`+` block handles (a horizontal 32×32 row), wasting ~20% of the width on mobile and making the editor look half-empty. Two changes: (1) the handle is now a compact **vertical grip** (stacked 26×26 items), shrinking the shown-state gutter from **88px → 48px** with *more* clearance than before (so the `+`/drag controls still never clip); (2) a new **toggle** in the Live toolbar hides the handles entirely and collapses the gutter to a normal text margin (**16px**) for full-width content — the slash `/` menu is a separate widget and keeps working with handles hidden. The choice persists per browser (`localStorage`) and defaults to hidden on **touch devices** (`hover: none` / `pointer: coarse`), shown on pointer devices. The shown gutter was tightened further to **36px**, and list indentation was reduced (marker column 24px→20px, bullet→text gap 10px→4px) so list text no longer sits far from the left.
