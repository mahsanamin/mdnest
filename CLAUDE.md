...
frontend/
  src/
    ...
    components/
      ...
      TaskBoard.jsx          # Kanban board over note checkboxes (v4.0.0+); lazy-loaded. v4.2.0+ adds the filter bar and the "All workspaces" scope. v4.2.2+: reached from the toolbar's `Editor | Board` switch (`.toolbar-view-switch`), which answers "what am I looking at"; the Basic/Live group answers "how am I editing it" and is hidden while the board is open. The board has lived inside the Basic/Live control (all three read as one choice) and in the sidebar (it read as the first row of the file tree, next to the root row) — don't move it back to either. Columns paint 100 cards at a time; server-side task scans are cached (backend/handlers/tasks_cache.go).
